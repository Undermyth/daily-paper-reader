...
title_zh: 探索大脑兴奋-抑制平衡的统一多尺度建模框架：应用于多发性硬化
...
tldr: 大脑兴奋-抑制平衡对动力学至关重要，其失衡与神经疾病相关。本文提出一个统一的多尺度建模框架，结合动态因果建模（DCM）与虚拟大脑（TVB），用于刻画大脑兴奋/抑制平衡，并应用于多发性硬化（MS）的视觉-运动子网络。结果显示，MS患者静息态下小脑至初级视觉皮层的连接由抑制转为兴奋，任务态下反馈连接随运动需求增加而失调，且与行为及临床指标相关。该框架整合连接级与网络级参数，揭示了MS中网络可塑性异常及其功能后果。
source: biorxiv
selection_source: fresh_fetch
motivation: 现有神经影像模型难以同时刻画多尺度兴奋-抑制失衡，缺乏从连接级到网络级的统一框架来研究脑疾病如多发性硬化中的机制。
method: 结合DCM（任务态与静息态fMRI）与TVB（扩散加权成像构建网络），分析MS患者与健康对照的视觉-运动子网络的有效连接与兴奋性增益。
result: MS中静息态小脑-视觉皮层连接由抑制变为兴奋；任务态反馈连接失调随运动负荷增加，且与反应时间和残疾程度相关；TVB显示NMDA增益降低导致更慢反应。
conclusion: 提出统一多尺度框架，揭示MS中兴奋/抑制失衡的层次性特征，为连接组与神经计算模型的整合提供新途径。
---

## 摘要
兴奋与抑制的平衡对大脑动力学至关重要，其失衡可导致神经系统疾病中的网络功能障碍。在此，我们提出一个概念上统一的多尺度脑建模框架，结合应用于任务和静息态功能磁共振成像（fMRI）数据的动态因果建模（DCM）以及虚拟大脑（TVB），来表征大脑的兴奋/抑制平衡。我们将该框架应用于一个由9名健康对照者和17名多发性硬化（pwMS）患者组成的队列中的视觉运动脑子网络。采集的数据包括具有可变握力的事件相关任务fMRI实验、静息态fMRI和弥散加权成像。视觉运动网络包含双侧初级视觉皮层（V1）、左侧初级运动皮层（M1）、辅助运动区和前运动皮层（SMAPMC）、扣带皮层（CC）、顶上小叶（SPL）以及右侧小脑小叶VI（CR）。DCM结果显示，虽然MS中整体网络架构得以保留，但有效连接（effective connectivity）的兴奋/抑制性质存在显著改变：在静息状态下，CR到V1的连接在统计上发生变化，在健康志愿者中为抑制性，而在MS中为兴奋性。在任务期间，有效连接反馈（包括小脑自连接）在健康志愿者中为正向，而在MS中为负向，并且随着运动需求的增加而日益失调。功能和有效连接的改变与行为表现（任务反应时间）和临床指标（残疾严重程度）相关。在整体组水平上，TVB参数将NMDA介导的兴奋性增益降低与较慢的任务反应联系起来。此外，整合DCM和TVB表明，更高的全局兴奋性增益与跨感觉运动和视觉运动通路的更强任务参与有效连接相关，从而将TVB捕获的网络级兴奋性与情境依赖的定向交互重配置以及DCM揭示的连接级强度联系起来。
