...
tldr: "推荐与广告系统中特征异质且交互稀疏，直接混合全部token会导致参数效率低。HubMixer引入少量可学习潜在hub，通过“归纳-交互-读出”范式组织特征交互：先以跨注意力将异质token归纳为紧凑hub，再在hub空间进行高阶交互，最后按token条件读回全局交互语义。离线实验超越现有最优模型，在线A/B测试在快手招聘业务中简历提交转化率提升5.48%，并已全量部署，体现了参数高效特征交互架构的工业价值。"
source: arxiv
...
motivation: 推荐特征语义空间异构、交互模式稀疏，直接混合全部token参数效率低，需要更高效的特征交互组织方式。
method: 通过潜在hub执行归纳-交互-读出流程，用跨注意力聚合异质token，在hub空间完成高阶交互后向原token读回信息。
result: "离线指标超越SOTA，在线A/B提升5.48%简历提交转化率，模型已上线生产系统。"
conclusion: HubMixer以参数高效方式实现了更优的特征交互，验证了潜在hub架构在推荐场景的可行性和工业落地能力。
---

## 摘要
学习有效的特征交互是工业推荐和广告排序系统的核心。最近的token混合架构通过轻量级混合算子简化了自注意力，提高了硬件效率并支持大规模部署。然而，推荐token本质上是异质的：用户画像、物品属性、行为序列、上下文特征、统计信号和业务侧特征位于不同的语义空间中，并以稀疏的、样本特定的模式进行交互。因此，在原始异质token空间中直接混合所有token可能参数效率低下，因为模型必须隐式发现哪些特征组应该交互以及这些交互应该如何路由。在本文中，我们提出了HubMixer，一种用于推荐中特征交互的参数高效潜在中心混合架构。HubMixer不直接混合原始特征token，而是引入一小组可学习的潜在中心，通过'归纳-交互-读出'范式来组织特征交互。首先，中心归纳将异质token总结为紧凑的潜在中心，其中潜在中心通过交叉注意力查询输入token。其次，中心交互在更干净的潜在中心空间中执行高阶交互。第三，token条件读出让每个原始token选择性地从交互后的中心读取，注入全局交互语义，同时保留token级字段身份。在工业推荐任务上的大量离线实验表明，HubMixer优于SOTA模型。在快手短视频招聘业务中的在线A/B测试进一步显示，简历提交转化率统计显著提高5.48%，HubMixer已全面部署在生产环境中。
